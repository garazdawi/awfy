...
3. ✅ **`bin/resolve-fill-needs.sh` simplification (plan §197).** Done.
   The script emits six per-(mode, platform) arrays (`targets_modern_{linux,macos,windows}`, `targets_legacy_{linux,macos,windows}`) plus the matching six `has_modern_*` / `has_legacy_*` booleans. Each measure-* job's `matrix.target` references the array for its (mode, platform) directly and gates the whole job on the matching `has_*` boolean — no per-row filtering inside the job at all.

   **GHA constraint we hit and walked back from (worth keeping noted):** the cleaner shape would be one unified `targets_<plat>` array per platform with each entry carrying `mode: "modern"|"legacy"`, then job-level `if: matrix.target.mode == '<mode>' && needs.resolve.outputs.has_<mode>_<plat> == 'true'`. This **does not work**: GHA evaluates job-level `if:` *before* matrix expansion, so `matrix.target` isn't in scope; naming it makes the entire workflow fail at parse time (instant `failure` with no jobs run, no useful error in the UI — see run `25431868728` head `9c69430e` if a future archaeologist wants to see the failure mode). Step-level `if:` *does* see matrix context, but spawning a runner just to skip every step costs ~30 s per wrong-mode row. Pre-filtering once in resolve sidesteps both. The intermediate unified-array shape was attempted as commit `9c69430e` and forward-fixed in the next push.
