...
describe("buildSeries", () => {
  ...
  it("inverts the whisker: yMin uses ms+2σ, yMax uses ms−2σ", () => {
    const series = dash.buildSeries(rows, "otp");
    const x86 = series.find(s => s.data.some(d => d.run_label.includes("x86_64")));
  ...
  it("caps the whisker at median/2 so a high-CV point can't blow up yMax", () => {
    // CV ~50% — without the cap, 2σ approaches median and yMax
    // shoots toward Infinity (this was the original 100× spike
    // bug on graphemes_bmp_4k OTP 24.2.2). The cap pins the
    // upper whisker at 2× the ratio.
    const noisy = [
      { lang: "erlang", machine_class: "linux-x86_64", emu_flavor: "jit",
        benchmark: "Noisy", otp: "27.0", median_ms: 1000, stddev_ms: 10,
        label: "27.0-test", timestamp: "2026-01-01T00:00:00Z", inner_iter: null },
      { lang: "erlang", machine_class: "linux-x86_64", emu_flavor: "jit",
        benchmark: "Noisy", otp: "28.0", median_ms: 100, stddev_ms: 50,
        label: "28.0-test", timestamp: "2026-02-01T00:00:00Z", inner_iter: null }
    ];
    const series = dash.buildSeries(noisy, "otp");
    const p = series[0].data[1];
    // Raw 2σ = 100, but median = 100 → cap at median/2 = 50.
    // yMax = 1000 / (100 - 50) = 20  (without cap it would be Infinity).
    expect(p.yMax).toBeCloseTo(20, 4);
  });
