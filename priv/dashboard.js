...
// Append a "Y axis" radio group to the .controls row at init time.
// Per-bench only — the suite chart's geomean has only one sensible
// y axis. Default "ms" for the absolute-timing view; "speedup" is
// what the chart used to default to and stays available.
function buildYAxisRadio(persisted, fallback) {
  const controls = document.querySelector(".controls");
  if (!controls) return;
  const group = document.createElement("div");
  group.className = "group";
  group.id = "control-yaxis";
  const heading = document.createElement("b");
  heading.textContent = "Y axis";
  group.appendChild(heading);
  const initial = (persisted === "ms" || persisted === "speedup") ? persisted : fallback;
  ["ms", "speedup"].forEach(v => {
    const lab = document.createElement("label");
    const inp = document.createElement("input");
    inp.type = "radio";
    inp.name = "yaxis";
  ...
  // Slot before the reset button so layout stays consistent.
  const reset = controls.querySelector(".reset-btn");
  if (reset) controls.insertBefore(group, reset);
...
function readUIState() {
  ...
  if (PAGE_KIND === "bench") {
    return {
      ...
      emu_flavor: flavor,
      lang,
      y_axis: (document.querySelector('input[name="yaxis"]:checked') || {}).value || "ms"
    };
  }
...
/* Build series: group rows by (lang, machine, arch, emu_flavor).
   ...
   yMode controls what hits the chart's y axis:
     - "speedup" — baseline_ms / current_ms, so higher = faster
       than the earliest run for that series. Whiskers invert as
       baseline/(ms±2σ) so the band still represents one σ in run
       time, not a symmetric speedup spread.
     - "ms" — raw median_ms with whiskers at median±2σ. Useful
       when comparing absolute timings across platforms (the
       speedup framing collapses platforms with very different
       baselines onto the same line).

   Either way, the data point keeps median_ms / base_ms / stddev so
   the tooltip can report both numbers regardless of axis mode.
*/
function buildSeries(rows, xAxis, yMode) {
  yMode = yMode || "speedup";
  const groups = {};
  rows.forEach(row => {
  ...
  return Object.entries(groups).map(([_key, items]) => {
    ...
    return {
      ...
      data: sorted.map(r => {
        ...
        let y, yMin, yMax;
        if (yMode === "ms") {
          y    = (typeof r.median_ms === "number") ? r.median_ms : null;
          yMin = (y !== null) ? Math.max(0, y - sigma) : null;
          yMax = (y !== null) ? y + sigma : null;
        } else {
          y    = ratio;
          yMax = (baseMs && r.median_ms - sigma > 0) ? baseMs / (r.median_ms - sigma) : ratio;
          yMin = (baseMs && r.median_ms + sigma > 0) ? baseMs / (r.median_ms + sigma) : ratio;
        }
        return {
          x,
          y,
...
function renderChart() {
  ...
  const xAxis = "otp";
  const state = readUIState();
  const yMode = PAGE_KIND === "bench" ? (state.y_axis || "ms") : "speedup";
  const series = PAGE_KIND === "suite"
    ? buildSuiteGeomeanSeries()
    : buildSeries(applyFilters(DATASET.rows, state), xAxis, yMode);
  ...
  const yLabel = PAGE_KIND === "suite"
    ...
    : (yMode === "ms" ? "median run time (ms, lower = faster)"
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: (ctx) => {
              const r = ctx.raw;
              let line = ctx.dataset.label + ": ";
              if (yMode === "ms" && typeof r.median_ms === "number") {
                line += r.median_ms.toFixed(2) + " ms";
                if (typeof r.ratio === "number") line += "  (" + r.ratio.toFixed(3) + "× over baseline)";
              } else {
...
(function () {
  ...
  // platforms checked and an absolute-ms y axis (most informative
  // when comparing platforms with very different baselines); suite
  // stays single-platform on the linux-x86_64 tab.
  ...
  if (PAGE_KIND === "bench") {
    ...
    buildYAxisRadio(state.y_axis, "ms");
  } else {
