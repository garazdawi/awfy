...
defmodule Mix.Tasks.Awfy.Compare do
  ...
  defp render_index(data, bench_names, baseline_label) do
    ...
    page_template(%{
      ...
      <p class="sub">Geometric mean of <code>baseline_median / median</code>, higher = faster. Per-platform lines are JIT-only and anchor at each platform's own earliest recorded run for every (lang × benchmark) — each line starts at 1× wherever its data begins. The thicker <strong>all platforms</strong> line groups by OTP function-release (e.g. <code>23.3</code>) so a coarser-grained Windows build (<code>OTP-23.3</code> installer) merges with a finer-grained linux/macos build (<code>OTP-23.3.4.20</code> patch tip) on the same OTP-23.3 line, and includes any bucket where at least one platform contributed. Pre-OTP-24 (no BeamAsm JIT) the line falls back to the emu rows, anchored to the same OTP-24.0 baseline as the JIT side, so the pre-/post-24 segments join cleanly. Chart is intentionally not configurable; toggle individual platforms via the legend.</p>
      """,
